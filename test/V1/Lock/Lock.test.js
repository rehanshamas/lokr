...
describe("Lock functional tests", () => {
  ...
  describe("Lock: Can add/remove beneficiaries. Equal Schedule. No restrictions to Split", () => {
    ...
    it("Should add new beneficiaries after initialization", async () => {  
      ...
      const amounts = calculateParts([12, 5]);
      Array.prototype.push.apply(initTokenAmounts, amounts);
      const amountToLock = convertToBN(12 + 5);
      ...
      expect(await token.balanceOf(lock.address)).to.be.equal(totalLockedAmount);
    });
    ...
    it("Should claim correctly amount after 2 days from untouched NFT", async () => {
      const idToClaim = IDs[2];
      const beneficiaryBalanceBeforeClaim = await token.balanceOf(account3.address);
      
      await time.increase(time.duration.days(2));
      const[, , claimable, ,] = await lock.getInfoBySingleID(idToClaim);

      await lock.connect(account3).claimUnlocked(idToClaim, []);
      
      expect(await token.balanceOf(account3.address)).to.be.equal(beneficiaryBalanceBeforeClaim.add(claimable));
    });
    it("Should claim correctly after a claim", async () => {
      ...
      expect(await token.balanceOf(account3.address)).to.be.equal(beneficiaryBalanceBeforeClaim.add(amountToClaim));
    });
    it("Should split correctly in two [50%, 50%]", async () => {
      const idToSplit = IDs[3];
      ...
      await lock.connect(account5).split(idToSplit, parts, toAddresses, []);
    ...
    it("Should revert when remove beneficiaries after first release", async () => {
    ...
    it("Should directly upgrade contract only by Governance Role", async () => {
      ...
      const ImpWithAcc6 = await ethers.getContractFactory('Lock', account6);
      expect(upgrades.upgradeProxy(lock.address, ImpWithAcc6))
        .to.be.revertedWith("ERROR: Upgrade not authorized");
      const newImplementation = await ethers.getContractFactory('Lock', governance);
