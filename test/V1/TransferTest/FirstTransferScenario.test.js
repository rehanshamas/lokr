...
describe("Lock First Scenario", () => {
    ...
    it("Should deploy Lock with canAddBeneficiaries = true & canTransfer = false, and no non-spitable part", async () => {
        // Prepare deploy fee
        ...
        let lockData =  ethers.utils.defaultAbiCoder.encode(
            ...
            [zeroAddress, zeroAddress, zeroAddress, token.address, governance, true, true, false, totalAmount]
        )
        ...
        expect(await lockProxy.connect(account1).canTransfer()).to.be.equal(false);
        ...
        await expect(lockProxy.connect(account4).transferFrom(account4.address, account5.address, 0)).to.be.revertedWith("ERROR: The lock policy do not allow transfers.");
    });
});