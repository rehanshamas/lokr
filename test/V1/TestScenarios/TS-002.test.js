...
describe("TS-002", () => {
  before(async () => {
    ...
    await token.transfer(governance.address, 600);
  });

  describe("Lock Scenario 002", () => {
    ...
    it("Should Claim after 10 days 1/2", async () => {
      await time.increase(time.duration.days(10));
      let holders = [A, B, C];
      let claimableAmounts = calculateParts([50, 100, 150]);
    ...
    it("Should be able to Claim small amount after 1 days", async () => {
      await time.increase(time.duration.days(1));
      let claimableAmounts = calculateParts([5, 10, 15]);
      let holders = [A, B, C];
      for (let i = 0; i < holders.length; i++) {
        const [, , claimable, ,] = await lock.getInfoBySingleID(i);
    ...
    it("Should Claim after last 10 days 2/2", async () => {
