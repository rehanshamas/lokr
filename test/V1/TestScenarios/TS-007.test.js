...
describe("TS-007", () => {
  ...
  describe("Lock Scenario 007", () => {
    ...
    it("Should deploy, init and add beneficiaries on initialization", async () => {
      ...
      const amounts = calculateParts([100, 200, 300]);
      const initLockedAmount = convertToBN(100 + 200 + 300);
      ...
      let shmKeyPoints =
        [
          parseInt(shmLockStart) + parseInt(time.duration.days(2)),
          parseInt(shmLockStart) + parseInt(time.duration.days(4)),
          parseInt(shmLockStart) + parseInt(time.duration.days(6))
        ];
      let shmAmounts = calculateParts([0.25, 0.42, 0.33])
    ...
    it("Should Verify that 600 TKN transfered from User to Newly-deploy lock", async () => {
      expect(await token.balanceOf(lock.address)).to.be.equal(convertToBN(600));
    });
    it("Should Claim after first 2 days 1/3", async () => {
    ...
    it("Should Claim after other 2 days 2/3", async () => {
      await time.increase(time.duration.days(2));
      let holders = [A, B, C];
      for (let i = 0; i < holders.length; i++) {
        await lock.connect(holders[i]).claimUnlocked(i);
    ...
    it("Should Claim after last 2 days 3/3", async () => {
      await time.increase(time.duration.days(2));
      let holders = [A, B, C];
      for (let i = 0; i < holders.length; i++) {
        await lock.connect(holders[i]).claimUnlocked(i);
