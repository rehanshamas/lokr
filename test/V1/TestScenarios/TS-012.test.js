...
describe("TS-012", () => {
  ...
  describe("Lock Scenario 012", () => {
    ...
    it("Should deploy, init and add beneficiaries on initialization", async () => {
      ...
      const amounts = calculateParts([59]);
      const initLockedAmount = convertToBN(59);
      ...
      let shmKeyPoints =
        [
          ...
          parseInt(shmLockStart) + parseInt(time.duration.days(59))
        ];
    ...
    it("Should Verify that 59 TKN transfered from User to Newly-deploy lock", async () => {
      expect(await token.balanceOf(lock.address)).to.be.equal(convertToBN(59));
    });
    ...
    it("Should Claim after last 59 days 2/2", async () => {
