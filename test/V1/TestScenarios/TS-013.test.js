...
describe("TS-013", () => {
    ...
    before(async () => {
        ...
        const deployFeeConfiguration = ethers.utils.defaultAbiCoder.encode(
            ...
            [ethers.utils.parseUnits("100", 'ether'), ethers.utils.parseUnits("0.01", "ether"), A.address, paymentTokens, tokenPriceFeed, cryptoPriceFeed, keccak256('FIXED_PAYMENT_OPTION')])
    ...
    it("Should deploy Lock with lock.canTransfer = true and no non-spitable part", async () => {
        ...
        let beneficiaries = [A.address, B.address, C.address];
        let amounts = [ethers.utils.parseEther('200'), ethers.utils.parseEther('300'), ethers.utils.parseEther('400')];
    ...
    it("First day", async () => {
        ...
        expect(await token.balanceOf(A.address)).to.be.equal(toWei(40));
        await lockProxy.connect(B).claimUnlocked(1, []);
        expect(await token.balanceOf(B.address)).to.be.equal(toWei(60));
        await lockProxy.connect(C).claimUnlocked(2, []);
        expect(await token.balanceOf(C.address)).to.be.equal(toWei(80));
    ...
    it("Second day", async () => {
        ...
        await time.increase(time.duration.days(1));
        await lockProxy.connect(A).claimUnlocked(0, []);
        expect(await token.balanceOf(A.address)).to.be.equal(toWei(40 * 2));
        await lockProxy.connect(B).claimUnlocked(1, []);
        expect(await token.balanceOf(B.address)).to.be.equal(toWei(60 * 2));
        await lockProxy.connect(C).claimUnlocked(2, []);
        expect(await token.balanceOf(C.address)).to.be.equal(toWei(80 * 2));
    ...
    it("Third day", async () => {
        ...
        await time.increase(time.duration.days(1));
        await lockProxy.connect(A).claimUnlocked(0, []);
        expect(await token.balanceOf(A.address)).to.be.equal(toWei(40 * 3));
        await lockProxy.connect(B).claimUnlocked(1, []);
        expect(await token.balanceOf(B.address)).to.be.equal(toWei(60 * 3));
        await lockProxy.connect(C).claimUnlocked(2, []);
        expect(await token.balanceOf(C.address)).to.be.equal(toWei(80 * 3));
