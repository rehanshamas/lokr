...
describe("TS-023", () => {
    ...
    it("last quarter", async () => {
        
        await time.increase(time.duration.weeks(4));        
        
        await lockProxy.connect(A).claimUnlocked(2,[]);
        expect(await token.balanceOf(A.address)).to.be.equal( toWei(18*7) );
        await lockProxy.connect(B).claimUnlocked(3,[]);
        expect(await token.balanceOf(B.address)).to.be.equal( toWei(2*7) );
        await lockProxy.connect(A).claimUnlocked(4,[]);
        expect(await token.balanceOf(A.address)).to.be.equal( toWei(18*9) );
        await lockProxy.connect(B).claimUnlocked(5,[]);
        expect(await token.balanceOf(B.address)).to.be.equal( toWei(2*9) );
