...
describe("TS-018", () => {
    ...
    it("Should deploy Lock with lock.canTransfer = true and no non-spitable part", async () => {
        ...
        let shmLockLength = time.duration.days(40*3); // month = 4 weeks
        let shmLockFirstClaimDelay = time.duration.days(40);
    ...
    it("First day", async () => {
        ...
        expect(await token.balanceOf(A.address)).to.be.equal( toWei(200).div(3) );
    ...
    it("Second day", async () => {
        ...
        await time.increase(time.duration.days(40));
        await lockProxy.connect(A).claimUnlocked(0,[]);
        expect(await token.balanceOf(A.address)).to.be.equal( toWei(200).mul(2).div(3) );
        await lockProxy.connect(B).claimUnlocked(1,[]);
    ...
    it("Third day", async () => {
        ...
        await time.increase(time.duration.days(40));
        await lockProxy.connect(A).claimUnlocked(0,[]);
        expect(await token.balanceOf(A.address)).to.be.equal( toWei(200) );
        await lockProxy.connect(B).claimUnlocked(1,[]);
