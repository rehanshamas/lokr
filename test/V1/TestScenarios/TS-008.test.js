...
describe("TS-008", () => {
  ...
  describe("Lock Scenario 008", () => {
    ...
    it("Should deploy, init and add beneficiaries on initialization", async () => {
      ...
      const amounts = calculateParts([100, 200]);
      const initLockedAmount = convertToBN(100 + 200);
      ...
      let shmKeyPoints =
        [
          ...
          parseInt(shmLockStart) + parseInt(time.duration.days(5))
        ];
    ...
    it("Should Verify that 300 TKN transfered from User to Newly-deploy lock", async () => {
      expect(await token.balanceOf(lock.address)).to.be.equal(convertToBN(300));
    });
    it("Should not be able to claim tokens", async () => {
    ...
    it("Should Claim after first 2 days 1/2", async () => {
      await time.increase(time.duration.days(1));
      let holders = [A, B];
      for (let i = 0; i < holders.length; i++) {
    ...
    it("Should Claim after last 3 days 2/2", async () => {
