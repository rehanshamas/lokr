...
describe("TS-022", () => {
    ...
    it("Should deploy Lock with lock.canTransfer = true and spitable part", async () => {
        ...
        let shmLockLength = time.duration.weeks(5*4);
        let shmLockFirstClaimDelay = time.duration.weeks(1*4);

        //Beneficiaries data for the Lock
        let beneficiaries = [A.address, B.address];
        let amounts = [ethers.utils.parseEther('100'), ethers.utils.parseEther('100')];
    ...
    it("First month", async () => {
        ...
        expect(await token.balanceOf(A.address)).to.be.equal( toWei(18*2) );
        await lockProxy.connect(B).claimUnlocked(5,[]);
        expect(await token.balanceOf(B.address)).to.be.equal( toWei(2*2) );
    ...
    it("Second month", async () => {
        ...
        await lockProxy.connect(A).claimUnlocked(2,[]);
        expect(await token.balanceOf(A.address)).to.be.equal( toWei(18*3) );
        await lockProxy.connect(B).claimUnlocked(3,[]);
        expect(await token.balanceOf(B.address)).to.be.equal( toWei(2*3) );
        await lockProxy.connect(A).claimUnlocked(4,[]);
    ...
    it("last month", async () => {
        ...
        await lockProxy.connect(A).claimUnlocked(2,[]);
        expect(await token.balanceOf(A.address)).to.be.equal( toWei(18*5) );
        await lockProxy.connect(B).claimUnlocked(3,[]);
        expect(await token.balanceOf(B.address)).to.be.equal( toWei(2*5) );
        await lockProxy.connect(A).claimUnlocked(4,[]);
        expect(await token.balanceOf(A.address)).to.be.equal( toWei(18*6) );
        await lockProxy.connect(B).claimUnlocked(5,[]);
        expect(await token.balanceOf(B.address)).to.be.equal( toWei(2*6) );
