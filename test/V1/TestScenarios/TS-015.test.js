...
describe("TS-015", () => {
    ...
    it("Should deploy Lock with lock.canTransfer = true and no non-spitable part", async () => {
        ...
        let shmLockLength = time.duration.weeks(5);
        let shmLockFirstClaimDelay = time.duration.weeks(1);

        //Beneficiaries data for the Lock
        let beneficiaries = [A.address, B.address];
        let amounts = [ethers.utils.parseEther('200'), ethers.utils.parseEther('300')];
