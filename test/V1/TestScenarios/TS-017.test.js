...
const two_parts_of_three = 66666666666666666666;
...
describe("TS-017", () => {
    ...
    it("Should deploy Lock with lock.canTransfer = true and no non-spitable part", async () => {
        ...
        let shmLockLength = time.duration.weeks(4*3); // month = 4 weeks
        let shmLockFirstClaimDelay = time.duration.weeks(4);

        //Beneficiaries data for the Lock
        let beneficiaries = [A.address, B.address];
        let amounts = [ethers.utils.parseEther('300'), ethers.utils.parseEther('200')];
    ...
    it("First day", async () => {
        ...
        expect(await token.balanceOf(B.address)).to.be.equal( toWei(200).div(3) );
    });
    ...
    it("Second day", async () => {
        ...
        await lockProxy.connect(B).claimUnlocked(1,[]);
        expect(await token.balanceOf(B.address)).to.be.equal( toWei(200).mul(2).div(3) );
    ...
    it("Third day", async () => {
        ...
        await lockProxy.connect(B).claimUnlocked(1,[]);
        expect(await token.balanceOf(B.address)).to.be.equal( toWei(200) );
