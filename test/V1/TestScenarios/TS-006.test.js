...
describe("TS-006", () => {
    before(async () => {
        [governance, A] = await ethers.getSigners();
    ...
    describe("Lock Scenario 006", () => {
        ...
        it("Should deploy, init and add beneficiaries on initialization", async () => {
            ...
            const amounts = calculateParts([500]);
            const initLockedAmount = convertToBN(500);
            ...
            let shmLockLength = parseInt(shmLockStart) + parseInt(time.duration.days(1));
            let shmLockCliff = parseInt(shmLockStart) + parseInt(time.duration.days(1));
        ...
        it("Should Verify that 500 TKN transfered from User to Newly-deploy lock", async () => {
            expect(await token.balanceOf(lock.address)).to.be.equal(convertToBN(500));
        });
        it("Should Claim after first day", async () => {
            await time.increase(time.duration.days(1));
            let holder = A;
            let claimableAmounts = convertToBN(500);
            const [, , claimable, ,] = await lock.getInfoBySingleID(0);
            expect(claimable).to.be.closeTo(claimableAmounts, convertToBN(0.01));
            await lock.connect(holder).claimUnlocked(0);
        });
