...
import "../interfaces/IUnlockSchedule.sol";
  ...
  * @dev 
  */
contract DepositManager is BaseGovernanceWithUserUpgradable, IDepositManager {
    ...
    event DepositManagerInitialized(address);
    ...
    function __DepositManager_init_unchained(ILock _lock) internal onlyInitializing {
        ...
        emit DepositManagerInitialized(address(this));
    }
     ...
     */
    function removeDeposits(uint256 id) external onlyLockContract {
     ...
     */
    function verifyOwnership(address /*beneficiary*/, bytes calldata /*data*/) external pure override returns(bool, uint) {
        return (false, 0);
    }
