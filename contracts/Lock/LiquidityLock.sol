...
contract LiquidityLock is BaseGovernanceWithUserUpgradable, ReentrancyGuardUpgradeable {
  ...
  struct UserInfo {
    EnumerableSetUpgradeable.AddressSet lockedTokens; // records all tokens the user has locked
  ...
  EnumerableSetUpgradeable.AddressSet private lockedTokens;
  mapping(address => TokenLock[]) public tokenLocks; //map univ2 pair to all its locks
  
  struct FeeStruct {
    uint256 ethFee; // Small eth fee to prevent spam on the platform
    IERC20Burnable secondaryFeeToken; // UNCX or UNCL
  ...
  EnumerableSetUpgradeable.AddressSet private feeWhitelist;
  
  address payable devaddr;

  event onDeposit(address lpToken, address user, uint256 amount, uint256 lockDate, uint256 unlockDate);
  event onWithdraw(address lpToken, uint256 amount);
  event LiquidityLockInitialized(address);

  modifier onlyOwner {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ERROR: Only the Owner");
    _;
  }

  function initialize(IUniswapV2Factory _uniswapFactory, address governanceAddress) external {
    ...
    emit LiquidityLockInitialized(address(this));
  }
  
  function setDev(address payable _devaddr) public onlyOwner {
    require(_devaddr != address(0), "Can not set Address 0");
    devaddr = _devaddr;
  }
  
  function setSecondaryFeeToken(IERC20Burnable _secondaryFeeToken) public onlyOwner {
    require(address(_secondaryFeeToken) != address(0), "Can not set address 0");
   ...
   */
  function setReferralTokenAndHold(IERC20 _referralToken, uint256 _hold) public onlyOwner {
    require(address(_referralToken) != address(0), "Can not set address 0");
  ...
  function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlock_date, address payable _referral, bool _fee_in_eth, address payable _withdrawer) external payable nonReentrant {
    ...
    SafeERC20.safeTransferFrom(IERC20(_lpToken), address(msg.sender), address(this), _amount);
    ...
    if (!feeWhitelist.contains(msg.sender)) {
      if (_fee_in_eth) { // charge fee in eth
        ...
        SafeERC20.safeTransferFrom(gFees.secondaryFeeToken, address(msg.sender), address(this), burnFee);
        if (gFees.referralPercent != 0 && _referral != address(0)) { // referral fee
          ...
          SafeERC20.safeApprove(gFees.secondaryFeeToken, _referral, referralFee);
          SafeERC20.safeTransfer(gFees.secondaryFeeToken, _referral, referralFee);
    ...
    SafeERC20.safeTransfer(IERC20(_lpToken), devaddr, liquidityFee);
