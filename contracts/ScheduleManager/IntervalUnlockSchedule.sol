...
contract IntervalUnlockSchedule is BaseGovernanceWithUserUpgradable, IUnlockSchedule {
    ...
    event IntervalUnlockScheduleInitialized(address);
    ...
    function __IntervalUnlockSchedule_init_unchained(ILock _lock, uint256[] memory _keyPoints, uint256[] memory _amounts) internal onlyInitializing {
        ...
        emit IntervalUnlockScheduleInitialized(address(this));
    }
    ...
    function nextRelease() external view returns(uint256 timestamp) {
        for(uint256 i; i < keyPoints.length; ) {
            if(keyPoints[i] > block.timestamp) return timestamp = keyPoints[i];
            unchecked {
                i++;
            }
        }
